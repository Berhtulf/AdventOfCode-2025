...
struct AdventOfCode: AsyncParsableCommand {
  ...
  func run() async throws {
    ...
    for challenge in challenges {
      ...
      if benchmark {
        var partOneTimings = [Duration]()
        for _ in 0...100 {
          partOneTimings.append(await run(part: challenge.part1, named: "Part 1"))
        }
        print("Part 1 took \(partOneTimings.minAndMax(), default: "N/A").")

        var partTwoTimings = [Duration]()
        for _ in 0...1 {
          partTwoTimings.append(await run(part: challenge.part2, named: "Part 2"))
        }

        print("Part 2 took \(partTwoTimings.minAndMax(), default: "N/A").")
        #if DEBUG
          print("Looks like you're benchmarking debug code. Try swift run -c release")
        #endif
      } else {
        _ = await run(part: challenge.part1, named: "Part 1")
        _ = await run(part: challenge.part2, named: "Part 2")
      }
    }
  }
}
